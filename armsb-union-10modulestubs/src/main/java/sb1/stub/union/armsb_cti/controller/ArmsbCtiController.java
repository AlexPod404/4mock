package sb1.stub.union.armsb_cti.controller;
...
import sb1.stub.union.armsb_cti.service.ArmsbCtiService;

import javax.servlet.http.HttpServletRequest;

@Slf4j
@RestController
@RequestMapping("/armsb_cti")
public class ArmsbCtiController {
    
    @Autowired
    private MockManagementService mockManagementService;
    
    @Autowired
    private ArmsbCtiService armsbCtiService;
    
    private static final String MOCK_ID = "armsb_cti";
    
    @GetMapping("/health")
    public ResponseEntity<?> health() {
        if (!mockManagementService.isMockEnabled(MOCK_ID)) {
            return ResponseEntity.status(503).body(ApiResponse.error(503, "Mock armsb_cti is disabled"));
        }
        
        return ResponseEntity.ok(ApiResponse.success("armsb_cti is healthy"));
    }
    ...
    public ResponseEntity<String> processData(@RequestBody String requestBody, HttpServletRequest request) {
        if (!mockManagementService.isMockEnabled(MOCK_ID)) {
            return ResponseEntity.status(503).body("{\"error\": \"Mock armsb_cti is disabled\"}");
        }
        
        log.info("armsb_cti - processData called with request: {}", requestBody);
    ...
    public ResponseEntity<String> getInfo(@PathVariable String id, HttpServletRequest request) {
        ...
        log.info("armsb_cti - getInfo called with id: {}", id);
    ...
    public ResponseEntity<String> updateData(@RequestBody String requestBody) {
        ...
        log.info("armsb_cti - updateData called");
