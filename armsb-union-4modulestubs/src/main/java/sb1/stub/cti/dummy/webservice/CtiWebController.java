package sb1.stub.cti.dummy.webservice;


import lombok.SneakyThrows;
import lombok.extern.slf4j.Slf4j;
import org.springframework.web.bind.annotation.*;
import sb1.stub.cti.dummy.data.hardcode.Jsons;

@Slf4j
@RestController
@RequestMapping("/cti")
public class CtiWebController {
    ...
    @SneakyThrows(Exception.class)
    @RequestMapping(value = "/getClientPhones", produces = "application/json")
    ...
    @SneakyThrows(Exception.class)
    @RequestMapping(value = "/call/init", produces = "application/json")
    ...
    @SneakyThrows(Exception.class)
    @RequestMapping(value = "/setNotification", produces = "application/json")
    ...
    public String setDelta(@PathVariable String delta) {
        CtiWebController.delta = Long.parseLong(delta, 10);
        return "Set delta to: " + CtiWebController.delta + "\n";
    }

    @RequestMapping(value = "/getDelta", method = RequestMethod.GET)
    @ResponseBody
    public String getDelta() {
        return "Delta is: " + CtiWebController.delta + "\n";
    }
}
